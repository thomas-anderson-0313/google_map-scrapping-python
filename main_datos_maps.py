...
def scrapearMaps(idioma, lista, outputFolder, resultados, hilo):
    ...
    for l in lista:
        ...
        if (lugar != None):
            print('Output nº '+str(hilo)+' ' + str(cont) +
                  '/' + str(len(lista)) + ' - OK - ' + l)
            listaLugares.append(lugar)
        else:
            print('Output nº '+str(hilo)+' ' + str(cont) +
                  '/' + str(len(lista)) + ' - ERROR - ' + l)
        cont += 1
...
def mainGoogleMaps(idioma, ficheroKw, outputFolder):
    ...
    for i in range(len(listaHilos)):
        listaHilos[i] = Thread(target=scrapearMaps, args=(
            idioma, divididos[i], outputFolder, listaResultados, i,))
        listaHilos[i].start()
...
if __name__ == "__main__":
    # while True:
    #     idioma = input('----------\n[1] Enter the language, (ES o EN): ')
    #     if(idioma != 'ES' and idioma != 'EN'):
    #         print("----------\n** Error ** Wrong password. Enter a valid one\n")
    #         continue
    #     else:
    #         break

    # while True:
    #     fichero = input('----------\n[2] Introduce the path to save the outputs: ')
    ...
    #         break
    idioma = "EN"
    kwLugares = "D:/2022-11-25-gMap-scrapping/google_maps_scraper/keywords.txt"
    fichero = "D:/2022-11-25-gMap-scrapping/google_maps_scraper/outputs"
